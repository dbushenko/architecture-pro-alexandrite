...
kubectl exec -it $(kubectl get pods -l app=service-a -o jsonpath='{.items[0].metadata.name}') -- wget --post-data='{"items": [{"id": "item1", "name": "Product A", "quantity": 2}]}' --header='Content-Type: application/json' -qO-  http://service-a:3000/orders
```
